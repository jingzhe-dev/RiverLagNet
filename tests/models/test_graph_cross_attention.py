...
def test_multiscale_path_attention_normalizes_all_ancestor_path_lag_candidates() -> None:
    module = EdgeLagHorizonSparseAttention(
        hidden_dim=8,
        edge_dim=2,
        num_heads=2,
    ...
    history = torch.randn(2, 7, 4, 8)
    queries = torch.randn(2, 4, 4, 8)
    edge_index = torch.tensor([[0, 1, 2], [3, 3, 3]])
    edge_attr = torch.tensor([[0.0, 3.0], [0.0, 2.0], [0.0, 1.0]])
    edge_hops = torch.tensor([3, 2, 1])

    contexts, weights = module(
        history, queries, queries, edge_index, edge_attr, edge_hops
    )

    assert contexts.shape == (2, 4, 4, 2, 4)
    assert weights.shape == (2, 4, 3, 5, 2)
    normalized = weights.sum(dim=(2, 3))
    assert torch.allclose(normalized, torch.ones_like(normalized), atol=1e-5)
