...
配对误差诊断进一步表明，新图修正与无图残差的相关系数为 `0.250`，高于旧 8 跳修正的 `0.132`，但训练期拟合的全局最优缩放仅为 `0.519`。应用该训练期缩放后，验证 macro NSE 为 `0.602054`；即使在同一验证集上按节点×提前期×目标拟合不可部署 oracle，也只有 `0.614667`，仍低于 15% 门槛 `0.677623`。目标增益分别为 NH3N `+0.011660`、CODMn `+0.005665`、TP `+0.000395`；提前期增益分别为 1–7 天 `+0.003208`、8–14 天 `+0.007013`、15–30 天 `+0.006584`。这说明下一机制应抑制上游绝对状态中的共同背景并保留相对传播创新，而不是继续放大当前图修正。
